...
- Vite/React frontend (`packages/web`) — **Partial**: the whole app now sits behind a real sign-in/create-account screen, with eleven real screens wired to `services/api` — Overview (executive KPI dashboard, Brand SOV/GRP ranking, TV/Cable TV/Radio split, Weak Inventory flags — the sidebar dashboard, not tied to any one project's drill-down), Projects (list/search/create — click a project to open its Project Detail page, see below), **Project Detail** (a new drill-down page, not a sidebar item — project summary, a Total GRPs/Spots/Spend/Matched-Activity KPI row, Share of Voice and Share of Expenditure side by side, Version History with restore, and the full Project Settings panel — edit every project field, and delete/detach uploads/brands/ratings — always visible here rather than behind a toggle), Ratings (upload — with the same mapping-template reuse — or attach from the library), Matches (review and confirm/reject fuzzy matches, manual assignment, recompute), Activity (spot-level GRP audit trail), Reports (station/programme/daypart contribution, spot efficiency, weekly trend, brand-vs-brand comparison), Spend Intelligence (media spend, Share of Expenditure, TV/Cable TV/Radio spend split, cost-per-GRP efficiency flagging — a new screen, not originally in this roadmap), Quality (validation issues), Settings (your account, plus team/role management for owner/admin), Exports (generate and download the full Excel workbook) — each with real loading/error/empty states, no sample data. A project can go from empty to a reviewed, calculated, analyzed, exported GRP entirely through the browser, as more than one real account.
...
| --- | --- | --- |
| Overview | Executive GRP/SOV dashboard, KPI cards, GRP-by-brand chart, SOV chart, TV vs Radio split, spot-volume-vs-GRP view | **Done** — now a genuinely dedicated screen (`packages/web/src/sections/OverviewSection.tsx`), split out of what used to be the Projects panel's dashboard-and-project-list conflation; the Projects tab is now purely list/search/create/archive/upload/calculate, matching its own label. Six KPI cards (Total GRPs, Total Spots, Matched Activity %, Cost per GRP, Leading Brand + its SOV **and** SOE, Total Spend) against `GET /runs/latest` + `/brand-shares`. GRP-weight and spend-weight are two side-by-side panels rather than one combined list: **Share of Voice** (GRP-by-brand ranking, TV/Cable TV/Radio split, proportional to `tvGrps`/`cableTvGrps`/`radioGrps`) and **Share of Expenditure** (spend-by-brand ranking, same TV/Cable TV/Radio split but for spend, reusing the identical row component Spend Intelligence's own ranking uses). A **Reading of the Period** panel synthesizes both into 2–3 plain-English sentences computed client-side from data already on screen — which brand converts spend into voice most efficiently, which brand pays the biggest premium per rating point (and which medium is driving it), and how Cable TV's spend share compares to its delivered-GRP share — only rendered when there's enough signal to say something (an all-null-cost project shows none of this). A Weak Inventory panel surfaces the top flagged buys from `GET /runs/{runId}/spot-efficiency`. Not literally a chart library — bars, same visual idiom as the rest of the app |
| Brands | Brand detail (GRP/SOV/spots by station, programme, day, week, daypart), brand-vs-brand comparison | Partial — brand-vs-brand comparison (GRP, SOV, spots, avg rating, TV/Radio split) is live in `packages/web`'s Reports screen against `GET /runs/{runId}/brand-shares`. Per-brand drill-down is now **Partial** too: a "Brand detail" selector on the Reports screen scopes Station Contribution, Programme Contribution, Daypart Contribution, Spot Efficiency, and Weekly Trend to one brand's own numbers (reusing the same fetched data, filtered client-side — no new endpoint needed for the drill-down itself). Spots by day specifically remains **Planned** — day isn't aggregated by any endpoint yet, only visible per-row on Activity. Daypart is now **Done** (see the Programmes row below) |
...
| Programmes | Programme and daypart contribution, top placements | Partial — programme contribution is live in `packages/web`'s Reports screen (`GET /runs/{runId}/programmes`), GRP ranked across all programmes. Daypart contribution is now **Done** too: `GET /runs/{runId}/dayparts` (new `grp_by_daypart` view, mirroring stations/programmes) groups by whatever daypart/time-band label the vendor's own file supplies (`media_activity.time_band`, e.g. "AM"/"Prime Time") rather than a canonical bucket this app invents — captured via a new `time_band` upload field, distinct from `programme` so one doesn't clobber the other's column detection. A distinct "top placements" view remains **Planned** |
...
| Ratings | Ratings Database for this project, plus access to the shared Ratings Library | Done — live in `packages/web`: upload a spreadsheet (parsed the same way brand reports are) or attach an existing dataset from the shared library, without leaving the project. Selecting a dataset now shows its actual rows in a table (`GET /api/ratings-datasets/{id}/rows`, already existed for the Matches screen's rating picker — just not rendered anywhere as a table until now), not only the summary card's row/issue counts; a flagged row (missing Medium/Station/Day, no Rating value, or a duplicate Medium+Station+Day+Programme key) expands on click to explain exactly why, naming the other row(s) it collides with for a duplicate |
...
All uploads are normalized to one internal structure — Brand, Medium, Station, Date, Day, Programme/Time Band, Spots — with optional Duration, Ad type, Start/End time, Campaign, Product, Source filename, Cost, and (new) Daypart.

**Daypart:** captured as its own field (`media_activity.time_band`), separate from Programme — a real vendor file often carries both (e.g. a "Time Belt" column alongside "Programme"). Deliberately captures the source file's own label as-is (AM/PM, "Prime Time", ...) rather than deriving canonical time-range buckets; see section 9's Programmes row for the aggregation this feeds.
...
- **Overview:** KPI cards (Total Category GRPs, Total Spots, Brands, Matched Activity %, Leading Brand, Leading Brand SOV, Total Spend), GRP-by-brand ranking chart, SOV chart (donut or horizontal bar), TV vs Cable TV vs Radio stacked view for multi-medium projects, spot-volume-vs-GRP view (flags brands buying weak inventory at high volume).
- **Brands:** per-brand detail (GRP, SOV, spend, SOE, spots, TV/Cable TV/Radio split, GRPs by station/programme/day/week/daypart, top placements) and a brand-vs-brand comparison table (GRPs, SOV, spend, SOE, spots, average rating, TV/Cable TV/Radio GRPs side by side).
- **Stations:** GRP contribution by station, per brand and overall.
- **Programmes:** GRP contribution by programme and daypart — grouped by whatever daypart/time-band label the source file itself supplies, not a fixed set of canonical buckets this app defines (or by programme genre).
...
The Overview KPIs and a GRP/SOV list are live (see the Screen Map row above), now with the TV/Cable TV/Radio split rendered as a segmented bar rather than just numbers in the caption — still a plain list, not a chart, and not a dedicated screen. Stations, Programmes, Daypart, Timeline/Trend, Spot Efficiency, and brand-vs-brand comparison are **Done**, live in a new **Reports** screen in `packages/web`: `GET /runs/{runId}/stations`, `/programmes`, and `/dayparts` (all three querying `grp_by_station`/`grp_by_programme`/`grp_by_daypart` Postgres views, or an equivalent grouped aggregation for the in-memory test backend) rank GRP contribution across all stations/programmes/daypart labels; `GET /runs/{runId}/spot-efficiency` flags weak-inventory-at-volume buys (computed on read from the station aggregate, not stored); `GET /runs/{runId}/trend` buckets matched activity by ISO week (`date_trunc('week', activity_date)`) per brand; brand-vs-brand comparison reuses the existing `brand-shares` data already fetched for Overview, letting the user pick any two brands for a side-by-side GRP/SOV/spend/SOE/spots/avg-rating/TV-Cable-Radio-split table. A "Brand detail" selector on the same screen now scopes Station/Programme/Daypart/Spot Efficiency/Trend to one brand — the per-brand drill-down piece of the Brands screen row above, built entirely client-side over data these panels already fetch. A single brand's GRPs by day/week specifically remain **Planned** — day isn't aggregated by any endpoint yet, only visible per-row on Activity.
...
- Full React replacement for the Streamlit UI, and retiring `app.py` — Export Centre, Project Settings, and Daypart Contribution above close three pieces of this; a distinct "top placements" view and actually retiring `app.py` remain
- **TV vs Terrestrial vs Cable TV granularity, Daypart contribution, and spot-volume-vs-GRP flagging** — **Done**, three roadmap items built together after the user asked to close out the Overview/Programmes gaps. `grp_calculator.normalize_medium_type()` gained a `CABLE TV` canonical bucket (DStv/GOtv/satellite/pay-TV aliases) alongside the existing `TV`/`RADIO`, additive to `brand_shares.cable_tv_grps` so `total_grps = tv_grps + cable_tv_grps + radio_grps` — existing data with a bare `TV` medium is unaffected, since that bucket's meaning didn't change. A new `time_band` upload field (separate from `programme` — a real collision with `detect_column`'s fuzzy substring matching was caught and fixed before shipping: an early synonym list included `daypart`/`day part`, which silently stole the plain `Day` column whenever a file had no real daypart column at all) captures the vendor's own daypart/time-band label as-is (AM/PM, "Prime Time", ...) rather than the app inventing canonical time-range buckets — a deliberate choice, since most files don't have a clean start-time column to derive them from. `GET /runs/{runId}/dayparts` (new `grp_by_daypart` view) aggregates by that label. `GET /runs/{runId}/spot-efficiency` (computed on read, not stored) flags a brand/station combo as weak inventory when its GRP-per-spot is under half the category average at 5+ spots. 16 new backend tests across both suites, plus a live smoke test through the real HTTP API confirming all three end to end on one upload (mixed TV/Cable TV/Radio spots with AM/PM daypart labels and one deliberately weak station).
- **A dedicated Overview screen, and a new Spend Intelligence screen** — **Done**. `packages/web`'s "Projects" tab used to conflate project list/management with the KPI-cards-and-Brand-SOV dashboard the Screen Map above always described as its own "Overview" screen; split out into `sections/OverviewSection.tsx` (own data fetch, same pattern as Reports/Activity/Quality — not duplicated state), leaving Projects to just list/search/create/archive/upload/calculate. Overview adds a Weak Inventory panel (top flagged buys from `GET /runs/{runId}/spot-efficiency`) alongside the moved KPI cards and Brand SOV/GRP ranking. **Spend Intelligence** is an entirely new screen, not previously in this roadmap: KPI cards (Total Spend, category Cost per GRP, Leading Spender + its SOE, Brands with spend data), a Spend-by-Brand ranking (same split-bar idiom as Brand SOV, sized by spend and split TV/Cable TV/Radio), and a Cost per GRP table flagging a brand as "Overpriced" when its cost-per-GRP sits more than 50% above the category average at 5+ spots — the mirror image of spot-efficiency's "weak inventory" flag (that one catches GRP too low for the spend; this one catches spend too high for the GRP). Backed by a new `brand_shares.tv_spend`/`cable_tv_spend`/`radio_spend` breakdown (additive, same shape as the GRP medium split, summed across every row regardless of match status like `total_spend` always has been) computed entirely from data already fetched for Overview/Reports — no new endpoint needed for the Spend Intelligence screen itself. 1 new backend test plus a live smoke test verifying the spend-by-medium split, then re-verified end to end after attaching ratings to confirm GRP-based flagging math against real computed numbers.
