...
| `media_activity` | The normalized spot-level rows produced after mapping — the audit trail's `source_file`/`source_row_number` live here. `cost` (media spend feature) is resolved at upload time from a direct `Cost` column or `Spots x Rate`; `null` when neither was ever mapped, independent of `rating_matches`/GRP — a row can have spend with no rating match at all. `time_band` (daypart contribution) is the vendor's own daypart/time-band label captured as-is, separate from `programme` — this column existed in the schema from Phase 1 but went unpopulated until the daypart feature; no migration needed to add it. `region` (SOE Explorer feature) is the vendor's own geography label (state/market/zone/territory), captured the same "as-is, blank if unmapped" way as `time_band` — a genuinely new column, unlike `time_band`, so it does need a migration; see the note below. |
...
| `grp_calculations` | Row-level GRP audit trail for a run — snapshots which `rating_match` was used, so later corrections don't rewrite history. Deliberately carries no cost/time_band column — neither is gated on a match the way GRP is, so they live on `media_activity`/`brand_shares` instead of here. |
| `brand_shares` | Per-run brand rollup (GRP, SOV, TV/Cable TV/Radio split, and Spend/SOE) backing the Overview and Spend Intelligence screens. `cable_tv_grps` is additive alongside the original `tv_grps` (still means terrestrial/generic TV) and `radio_grps` — `total_grps` sums all three. `total_spend`/`soe` are summed from **every** `media_activity` row for the brand regardless of match status — the one place this table's numbers don't all come from the same row population. `tv_spend`/`cable_tv_spend`/`radio_spend` break `total_spend` out the same way GRP is split, backing Spend Intelligence's medium breakdown. |
| `grp_by_station` / `grp_by_programme` / `grp_by_daypart` | Views, not tables — cheap aggregates over `grp_calculations` for the Stations/Programmes screens. `grp_by_daypart` groups by `media_activity.time_band`, the vendor's own label rather than a canonical bucket this app defines. No need to materialize until they're a proven bottleneck. |
