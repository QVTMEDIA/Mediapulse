...
| --- | --- | --- |
| Overview | Executive GRP/SOV dashboard, KPI cards, GRP-by-brand chart, SOV chart, TV vs Radio split, spot-volume-vs-GRP view | Partial — the KPI cards and a GRP/SOV list (not yet a chart) are live in `packages/web`'s Projects panel against `GET /runs/latest` + `/brand-shares`, with a "Calculate" button. TV vs Radio split is now **Done**, and extended to a three-way TV/Cable TV/Radio split: each brand's bar splits into segments proportional to `tvGrps`/`cableTvGrps`/`radioGrps`, with a legend, for any project with more than one medium present — a single-medium brand still shows one solid color. spot-volume-vs-GRP is now **Done** too: `GET /runs/{runId}/spot-efficiency` flags a brand/station combo as buying weak inventory when its own GRP-per-spot sits under half the category's average for the run, at 5+ spots so a single unlucky placement doesn't trigger it — computed on read, not stored, since "weak" is a judgment call; live in the Reports screen's new Spot Efficiency panel. Still not a dedicated Overview screen — both pieces live in the existing Projects/Reports panels |
