...
7. Replace local SQLite with Supabase/Postgres before multi-user production use — schema and migration path are **ready** (`db/schema.sql`, `db/README.md`); actually running against a managed instance rather than local `docker-compose` is **not done** (see `PRODUCT_ROADMAP.md` Phase 3).
