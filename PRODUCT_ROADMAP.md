...
| --- | --- | --- |
| Overview | Executive GRP/SOV dashboard, KPI cards, GRP-by-brand chart, SOV chart, TV vs Radio split, spot-volume-vs-GRP view | **Done** — now a genuinely dedicated screen (`packages/web/src/sections/OverviewSection.tsx`), split out of what used to be the Projects panel's dashboard-and-project-list conflation; the Projects tab is now purely list/search/create/archive/upload/calculate, matching its own label. KPI cards (Total GRPs, Total Spots, Matched Activity %, Leading Brand + its SOV, Total Spend) against `GET /runs/latest` + `/brand-shares`; GRP-by-brand ranking, SOV, and the TV/Cable TV/Radio split (extended from the original TV/Radio split, proportional to `tvGrps`/`cableTvGrps`/`radioGrps`) all live in one Brand SOV panel; a Weak Inventory panel surfaces the top flagged buys from `GET /runs/{runId}/spot-efficiency` (a brand/station combo buying weak inventory when its GRP-per-spot sits under half the category average at 5+ spots — computed on read, not stored, since "weak" is a judgment call). Not literally a chart library — bars, same visual idiom as the rest of the app |
