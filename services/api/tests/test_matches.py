...
def test_soe_explorer_uploads_are_invisible_to_matches(client, project, brand):
    # SOE Explorer uploads go through /api/soe/uploads entirely, never
    # /api/projects/{projectId}/uploads -- they can't reach this project's
    # Matches at all, not because of a flag, but because they were never
    ...
    soe_media = b'Brand,Channel,Programme,Day,Spots\nBrand X,TVC,Prime Time,Monday,7\n'
    soe_response = client.post('/api/soe/uploads', files={'file': ('soe.csv', io.BytesIO(soe_media), 'text/csv')})
    assert soe_response.status_code == 201
