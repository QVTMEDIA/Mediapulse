...
| `brand_shares` | Per-run brand rollup (GRP, SOV, TV/Cable TV/Radio split, and Spend/SOE) backing the SOV chart and brand comparison screen. `cable_tv_grps` is additive alongside the original `tv_grps` (still means terrestrial/generic TV) and `radio_grps` — `total_grps` sums all three. `total_spend`/`soe` are summed from **every** `media_activity` row for the brand regardless of match status — the one place this table's numbers don't all come from the same row population. |
